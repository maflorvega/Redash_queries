/*
Name: Home Hero Impressions by Brokerage
Data source: 4
Created By: Admin
Last Update At: 2016-04-01T14:27:59.095340+00:00
*/
...
FROM
  (SELECT H_Lis.marketingGroup_name AS Marketing_group,
   ...
   nvl(H_Lis.listing_address,
       H_Lis_address.street_address) AS Listing_address,
   nvl(MG_LA_US.STATE,'') AS STATE, 
   count(*) AS Impressions, nvl(Integer(L.Clicks),0) AS Clicks
   FROM
     (SELECT post_prop34,
