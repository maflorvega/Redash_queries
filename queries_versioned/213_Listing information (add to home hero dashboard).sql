/*
Name: Listing information (add to home hero dashboard)
Data source: 4
Created By: Admin
Last Update At: 2016-02-15T19:54:00.009243+00:00
*/
 (SELECT Date(date_time) as date,string(post_prop26) AS Listing,
          count(*) as amount
   FROM (TSELECT marketingGroup,
       Brokerage,
       Branch,
       Agent,
       Listing,
...
ORDER BY Views DESC,
         ...
         Branch,
         Agent,
         ListingABLE_QUERY(djomniture:cipomniture_djmansionglobal,'CONCAT(REPLACE(table_id,"_","-"),"-01") BETWEEN STRFTIME_UTC_USEC("{{startdate}}", "%Y-%m-01") and STRFTIME_UTC_USEC("{{enddate}}", "%Y-%m-31")'))
