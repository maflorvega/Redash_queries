/*
Name: Listing Views by MG details(mgid-dates)2
Data source: 4
Created By: Admin
Last Update At: 2016-10-21T17:27:50.930144+00:00
*/
   ...
   FROM
     (SELECT marketingGroup,
        ...
        (SELECT MG_HL.marketingGroup_name AS marketingGroup,
         ...
         FROM
           (SELECT nvl(L.Listing,v.Listing) AS Listing,
            ...
            OUTER JOIN EACH
              (SELECT string(id),created_at AS Listing
      ...
      GROUP BY marketingGroup,
               ...
               mgid,
               Listing) a
   left JOIN [djomniture:devspark.MG_Listing_Address] AS Lis_Det ON Listing = Lis_Det.id) b /*ADDING THE LEADS*/
LEFT OUTER JOIN
